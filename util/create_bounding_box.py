...
def ego_roi(frame_2, name, box_coord, DIST_H_per_FRAME):
    ...
    frame_2, coords_in_roi, distance_in_roi, names_in_roi = object_in_roi(frame_2, name, box_coord, x_1_low, x_2_low, y_up, y_low, DIST_H_per_FRAME)

    return frame_2, coords_in_roi, distance_in_roi, names_in_roi
...
def object_in_roi(frame_2, name, box_coord, x_1_low, x_2_low, y_up, y_low, DIST_H_per_FRAME):
    ...
    for i in range(length):
        ...
        if (obj_x_centre >= x_1_low) and (obj_x_centre <= x_2_low) and (obj_y_centre >= y_up) and (len(DIST_H_per_FRAME) != 0):
            ...
            names_in_roi.append(name[i] if i < len(name) else "obstacle")
            cv2.rectangle(frame_2, (box_coord[i][0], box_coord[i][1]), (box_coord[i][2], box_coord[i][3]), (0, 255, 255), 3)
        else:
            cv2.rectangle(frame_2, (box_coord[i][0], box_coord[i][1]), (box_coord[i][2], box_coord[i][3]), (0, 255, 0), 3)
